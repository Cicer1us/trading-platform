...
import walletFacetsDeployArgs from './params';

const owner = process.env.GNOSIS_SAFE_ADMIN;

async function deploy() {
  const network = await ethers.provider.getNetwork();
  const deploymentArgs = walletFacetsDeployArgs[network.chainId];
  if (!deploymentArgs) {
  ...
  console.log('');
  console.log('Deploying CrossChainWallet facets (without initial data):');
  const crossChainRouterFacetNames = ['DestCrossSwapFacet'];
  for (const facetName of crossChainRouterFacetNames) {
    await deployFacet(facetName, cuts);
  }

  console.log('');
  console.log('Deploying CrossChainWallet Diamond...');
  const Diamond = await ethers.getContractFactory('CrossChainWallet');
  const diamond = await Diamond.deploy(cuts, owner, { gasLimit: '0x2dc6c0' });
  await diamond.deployed();

  console.log('CrossChainWallet deployed:', diamond.address);

  console.log('');
  console.log('Deploying CrossChainWallet Facets with initial data...');

  for (const facet of walletFacetsDeployArgs[network.chainId]) {
