...
SwapFilterSidebarEthereum.propTypes = {
...
export default function SwapFilterSidebarEthereum({
  isOpenFilter,
  onResetFilter,
  onOpenFilter,
  onCloseFilter,
  ...
  const getTokens = async (params = {}) => {
    try {
      const swapResponse = await axios.request({
        ...
        params: { ...params, chainId: ChainId.Ethereum }
      });

      const limitResponse = await axios.request({
        method: 'get',
        url: `/limit-order/traded-tokens`,
        baseURL: process.env.REACT_APP_SERVER_URL,
        params: {
      ...
      const addresses = new Set(swapResponse.data.map((d) => d.address));
      const merged = [
        ...swapResponse.data,
        ...limitResponse.data.filter((d) => !addresses.has(d.address))
      ];
      setTokens(merged);
    } catch (err) {
      //
